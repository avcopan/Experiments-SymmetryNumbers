...
GeneratedBondStereo = (
    NonRotatableBond33 | NonRotatableBond23 | NonRotatableBond13
)
...
def external_symmetry_number(graph: StereoMolGraph) -> int:
    ...
    bond_auto_int: dict[Bond, int] = {
        bond: hash(frozenset(bond_set))
        for bond, bond_set in bond_auto_set.items()
    }
    ...
    def _ordered_three_neighbors(
        ...
        *,
        ...
        if isinstance(stereo, Tetrahedral):
            for stereo_atoms in stereo._perm_atoms():
                ...
                if (
                    ...
                    and stereo_atoms[2] != unique_neighbor
                ):
                    continue
        ...
        neighbors = tuple(
            sorted(n for n in graph.bonded_to(atom) if n != bonded_atom)
        )
    ...
    def _map_non_rotatable_state(
        ...
        return state.__class__(
            atoms=tuple(mapping[a] for a in state.atoms), parity=1
        )
    ...
    for bond_auto_cls, bonds in int_bond_auto_set.items():
        ...
        for mapping in mappings:
            ...
            mapped_state = _map_non_rotatable_state(
                representative_state, mapping
            )
    ...
    for mapping_id, mapping in enumerate(mappings):
        for bond_state_dict in counter.values():
            for bond_state, mapping_ids in bond_state_dict.items():
                if all(
                    _map_non_rotatable_state(non_rot_bond, mapping)
                    in bond_state
...
def old_external_symmetry_number(graph: StereoMolGraph) -> int:
    ...
    int_bond_auto_set: dict[int, set[Bond]] = {
        auto_cls_int: bond_auto_set[b]
        for b, auto_cls_int in bond_auto_int.items()
    }

    counter: dict[
        BondAutoCls, dict[frozenset[NonRotatableBond], set[MappingId]]
    ] = {}
        ...
        for bond in bonds:
            ...
            if (
                ...
                unique_left_int = next(
                    v
                    for v, c in Counter(
                        atom_auto_int[x] for x in nbrs1
                    ).items()
                    if c == 1
                )
                unique_right_int = next(
                    v
                    for v, c in Counter(
                        atom_auto_int[x] for x in nbrs2
                    ).items()
                    if c == 1
                )
                unique_left = next(
                    nbr
                    for nbr in nbrs1
                    if atom_auto_int[nbr] == unique_left_int
                )
                unique_right = next(
                    nbr
                    for nbr in nbrs2
                    if atom_auto_int[nbr] == unique_right_int
                )
                ...
                right = next(
                    ...
                    and stereo_atoms[2] == unique_right
                )

                nrb_set.add(
                    NonRotatableBond(atoms=(*left, a1, a2, *right), parity=0)
                )
    ...
    sym_number = 1
    for combo in itertools.product(*mapping_sets_by_cls):
        intersect: set[MappingId] = (
            set.intersection(*combo) if combo else set()
        )
